...
const API_BASE_URL = import.meta.env.VITE_API_ENDPOINT || 'http://localhost:5002/api';
...
const Dashboard: React.FC<DashboardProps> = ({ onLogout }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (foodCategoriesResponse?.ok) {
        const data = await foodCategoriesResponse.json();
  ...
  const renderRoomsTab = () => (
    <div className="space-y-6">
      ...
      {/* Individual Rooms */}
      <div>
        <h3 className="text-xl font-semibold mb-4">Individual Rooms</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {rooms.map((room) => (
  ...
  const renderRestaurantTab = () => (
    <div className="space-y-6">
            ...
            <div key={category._id} className="border rounded-lg p-4">
              <div className="flex justify-between items-start">
                ...
                <div className="flex gap-1">
                  <button
                    onClick={() => {
                      setEditingFoodCategory(category);
                      setShowFoodCategoryModal(true);
                    }}
                    className="bg-blue-500 text-white p-2 rounded hover:bg-blue-600"
                  >
                  ...
                  <button
                    onClick={() => handleDeleteItem(category._id, 'food-category', category.name)}
                    className="bg-red-500 text-white p-2 rounded hover:bg-red-600"
                  >
                    <Trash2 className="h-4 w-4" />
                  </button>
      ...
      <p className="text-gray-600 bg-blue-50 p-4 rounded-lg">
        Menu management interface coming soon... You can currently view and manage menu items below.
      </p>
            ...
            <div key={item._id} className="bg-white rounded-lg shadow p-4">
              <div className="flex justify-between items-start mb-2">
                ...
                <div className="flex space-x-1">
                  ...
                  <button
                    onClick={() => handleDeleteItem(item._id, 'menu-item', item.name)}
                    className="text-red-500 hover:text-red-700"
                  >
                    <Trash2 className="h-4 w-4" />
                  </button>
              ...
              <div className="flex justify-between items-center mb-2">
                ...
                <div className="flex space-x-1">
                  {item.isVegetarian && <span className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded">V</span>}
                  {item.isVegan && <span className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded">VG</span>}
                  {item.isGlutenFree && <span className="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded">GF</span>}
                  {item.isSpicy && <span className="px-2 py-1 bg-red-100 text-red-800 text-xs rounded">🌶️</span>}
                </div>
              </div>
